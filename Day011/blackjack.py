...
def initial_hands():
    ...
    while sum(initial_player_hand) != 22 and sum(initial_dealer_hand) != 22:
...
def show_score(both_hands):
    ...
    print(f"Conputer's cards {both_hands["dealer"]}, current score: {sum(both_hands["dealer"])}")


def play_again():
    again = input("Do you want to play a game of Blackjack? Type 'y' or 'n': ")
    return True if again.lower() == 'y' else False
